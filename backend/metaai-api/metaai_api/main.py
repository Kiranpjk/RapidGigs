...
class MetaAI:
    ...
    def _load_cookies_from_env(self) -> Optional[Dict[str, str]]:
        ...
        - META_AI_COOKIE_DATR: Required - Device identifier cookie
        ...
        """
        required_cookies = {
            "datr": os.getenv("META_AI_COOKIE_DATR"),
        ...
        **kwargs
        ...
        try:
            response = self.generation_api.generate_video(
                prompt=prompt,
                fetch_urls=True,  # Poll for actual video URLs during generation
            ...
            # If we got video_urls from fetch_urls polling, use them directly
            actual_video_urls = [
                v.get('url') for v in response.get('videos', [])
                if isinstance(v, dict) and v.get('url')
            ]
            actual_video_objects = response.get('videos', [])
